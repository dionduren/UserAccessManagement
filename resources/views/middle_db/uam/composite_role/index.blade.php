    ...
    <div class="container-fluid">
        <div class="card shadow-sm">
            <div class="card-header d-flex flex-column flex-md-row align-items-md-center gap-2">
                <h2 class="mb-0 flex-grow-1">Composite Roles</h2>
                <div class="d-flex gap-2">
                    <div hidden>
                        <input id="compLike" class="form-control form-control-sm" style="width:140px" value="ZM-%"
                            title="LIKE">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const table = $('#mainTable').DataTable({
                ...
                ajax: {
                    url: '{{ route('middle_db.uam.composite_role.data') }}'
                },
                columns: [{
                        data: 'id',
                        visible: false
            ...
            syncBtn.addEventListener('click', async () => {
                ...
                try {
                    const resp = await fetch('{{ route('middle_db.uam.composite_role.sync') }}', {
                        method: 'POST',
                        headers: {
                        ...
                        body: JSON.stringify({
                            like: document.getElementById('compLike').value || 'ZM-%',
                            exclude: document.getElementById('compExclude').value ||
                                '%-AO'
                        })
                    });
