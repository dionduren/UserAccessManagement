    ...
    <div class="container-fluid">
        <div class="card shadow-sm">
            <div class="card-header">
                <h4 class="mb-0">Create Composite Authorization Object</h4>
            </div>
            <div class="card-body">
                @if ($errors->any())
                ...
                <form action="{{ route('composite_ao.store') }}" method="POST" id="createForm">
                    @csrf
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="company_id" id="company_id" class="form-select" required>
                                ...
                                @foreach ($companies as $company)
                                    <option value="{{ $company->company_code }}" @selected(old('company_id') === $company->company_code)>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="composite_role_id" id="composite_role_id" class="form-select" required disabled>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const aoNameInput = document.getElementById('ao_name');
            ...
            companySelect.addEventListener('change', function() {
                ...
                fetch(`{{ route('composite-single.filter-company') }}?company_id=${companyId}`)
                    .then(r => r.json())
                    .then(data => {
                        compositeRoleSelect.innerHTML =
                            '<option value="">-- Select Composite Role --</option>';
                        ...
                        $(compositeRoleSelect).on('select2:select', function(e) {
                            const selectedText = e.params.data.text;
                            if (selectedText && selectedText !==
                                '-- Select Composite Role --') {
                                aoNameInput.value = selectedText + '-AO';
                            }
                        });

                        // Clear AO name when cleared
                        $(compositeRoleSelect).on('select2:clear', function() {
                            aoNameInput.value = '';
                        });
                    })
                    .catch(() => {
